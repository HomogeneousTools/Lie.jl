...
Irreducible character `m · V(λ)`. Requires `λ` to be dominant.
"""
...
Base.isone(V::WeylCharacter{DT,R}) where {DT,R} =
    length(V.terms) == 1 && haskey(V.terms, zero(WeightLatticeElem{DT,R})) && V.terms[zero(WeightLatticeElem{DT,R})] == 1
...
Compute the weight multiplicities of the irreducible representation `V(λ)`
...
[`freudenthal_formula`](@ref)) with the irreducible representation `V(μ)`,
using the Brauer–Klimyk formula:

``V \\otimes V(μ) = \\sum_{\\text{weights } λ \\text{ of } V} m(λ) \\cdot ε(λ+μ) \\cdot V(ν(λ+μ))``

where `(ε, ν) = dot_reduce(μ + λ)`.
"""
function brauer_klimyk(char::Dict{SVector{R,Int}, Int}, μ::WeightLatticeElem{DT,R}) where {DT,R}
...
Decompose the tensor product `V(λ) ⊗ V(μ)` into irreducibles using the
...
Dual of a virtual character: each summand `V(λ)` maps to `V(λ*)`.
"""
...
Compute the `k`-th Adams operator `ψᵏ(V(λ))`, returned as a dictionary of
weight multiplicities (not decomposed into irreducibles).

The Adams operator scales every weight by `k`: if `V(λ)` has weight
multiplicity `m(μ)`, then `ψᵏ(V(λ))` has `m(μ)` at weight `kμ`.
"""
...
Compute the `k`-th symmetric power `Symᵏ V(λ)` of the irreducible
representation with highest weight `λ`, using the Newton–Girard formula:

``k \\cdot Symᵏ(V) = \\sum_{r=1}^{k} ψʳ(V) \\cdot Symᵏ⁻ʳ(V)``
...
Compute the `k`-th exterior power `⋀ᵏ V(λ)` of the irreducible
representation with highest weight `λ`, using the Newton–Girard formula:

``k \\cdot ⋀ᵏ(V) = \\sum_{r=1}^{k} (-1)^{r-1} ψʳ(V) \\cdot ⋀ᵏ⁻ʳ(V)``
...
"""
function character_from_weights(::Type{DT}, multiplicities::Dict{SVector{R,Int}, Int}) where {DT<:DynkinType,R}
